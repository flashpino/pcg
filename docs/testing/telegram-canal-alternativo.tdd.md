...
**Commits:** `ed8c44f` (Task 1) → `ebcc33f` (Task 2) → `ba1f4cb` (Task 3) → `ba1f4cb` (Task 4) → `254d0e0` (Task 5) → `5437a89` (Task 6)
...
| 4 | Como contato, devo poder linkar meu chat_id via `/start` no bot (webhook da Telegram Bot API) | `cd server && npx vitest run src/routes/telegramWebhook.test.ts` (Task 4: 5 testes novos, 1 novo arquivo) | PASS (6 tests, 5 novos) |
...
| 8 | `enqueueTelegram(job)` enfileira mensagem na fila `notify-telegram` | `server/src/services/notifier.test.ts` (Task 2: função exportada e usável) | unit | PASS |
| 9 | `sendTelegram(job)` chama Bot API corretamente com `chat_id` e `text` (texto puro, sem `parse_mode`) | `server/src/services/notifier.test.ts` (implícito: worker roda em startNotifier) | integration | PASS |
...
- `src/routes/telegramWebhook.test.ts` (6 testes, +5 novos em Task 4, novo arquivo)
...
- **`web/src/pages/SupervisorsPage.tsx` está com conteúdo binário corrompido no disco local** (hash diferente entre HEAD e working tree, não detectado por `git status`/`git diff`), bloqueando `npm run build` em `web/`:
  ```
  error TS1005: ':' expected.
  ```
  Não editado, não corrigido — fora de escopo desta feature e de escopo de teste (Task 6 valida só com esbuild isolado, que não baixa dependências cruzadas).

## Resumo de evidência

- ✓ Todas as 168 testes passam (incluindo 16 novos Telegram)
